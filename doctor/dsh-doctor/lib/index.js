...
//#endregion
//#region src/index.ts
function environmentPath(environment, key, fallback) {
...
function validateProfileName(profile) {
	if (profile.length === 0 || profile === "." || profile === ".." || isAbsolute(profile) || /[\\/]/.test(profile)) throw new Error(`invalid profile name: ${profile}`);
}
function resolveDoctorPaths(profile, environment = process.env) {
	validateProfileName(profile);
	const home = resolve(environmentPath(environment, "DSH_DOCTOR_HOME", environmentPath(environment, "DSH_HOME", join(homedir(), ".dsh"))));
	const profileRoot = resolve(environmentPath(environment, "DSH_DOCTOR_PROFILE_ROOT", join(home, "profiles", profile)));
	...
	return {
		home,
		profileRoot,
		profileManifestPath: resolve(environmentPath(environment, "DSH_DOCTOR_PROFILE_MANIFEST", join(profileRoot, "package.json"))),
		lockfilePath: resolve(environmentPath(environment, "DSH_DOCTOR_LOCKFILE", join(profileRoot, "pnpm-lock.yaml"))),
		workspacePolicyPath: resolve(environmentPath(environment, "DSH_DOCTOR_WORKSPACE_POLICY", join(profileRoot, "pnpm-workspace.yaml"))),
		cordisPatchPath: resolve(environmentPath(environment, "DSH_DOCTOR_CORDIS_PATCH", join(profileRoot, "cordis.patch.yml"))),
		tokenMeterPackagePath: resolve(environmentPath(environment, "DSH_DOCTOR_TOKEN_METER_PACKAGE", join(home, "node_modules/@deepseek-ai/dsh-token-meter/package.json"))),
		cachePaths: cacheOverride === void 0 ? [join(storageRoot, "session_projcache.json")] : listPaths(cacheOverride).map((path) => resolve(path)),
		sessionRecordPaths: listPaths(sessionOverride).map((path) => resolve(path))
	};
}
function cliIO() {
	return { commandRunner: defaultCommandRunner };
}
async function runDoctorCli(argv = process.argv.slice(2), environment = process.env, io = cliIO()) {
	let parsed;
	try {
	...
	try {
		...
		if (parsed.command === "check") return runCheck(profileInput, io);
		if (parsed.command === "report-token-meter") return runTokenMeterReport({
...
if (invokedAsCli()) runDoctorCli().then((code) => {
...
//#endregion
export { createBackup, defaultCommandRunner, inspectProfile, inspectTokenMeter, parseDoctorArgs, redactSecrets, resolveDoctorPaths, runCheck, runDoctorCli, runProfileUpdate, runTokenMeterReport, sha256, sha256File };
